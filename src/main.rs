...
impl Board {
    fn new() -> Self {
        ...
        for (x, &pt) in back_row.iter().enumerate() {
            cells[0][x] = Some(Piece { color: Color::Black, piece_type: pt });
        }
        cells[2][1] = Some(Piece { color: Color::Black, piece_type: PieceType::Cannon });
        cells[2][7] = Some(Piece { color: Color::Black, piece_type: PieceType::Cannon });
        for x in [0, 2, 4, 6, 8].iter() {
            cells[3][*x] = Some(Piece { color: Color::Black, piece_type: PieceType::Pawn });
        }

        // 初始化红方 (y=5-9)
        for (x, &pt) in back_row.iter().enumerate() {
            cells[9][x] = Some(Piece { color: Color::Red, piece_type: pt });
        }
        cells[7][1] = Some(Piece { color: Color::Red, piece_type: PieceType::Cannon });
        cells[7][7] = Some(Piece { color: Color::Red, piece_type: PieceType::Cannon });
        for x in [0, 2, 4, 6, 8].iter() {
            cells[6][*x] = Some(Piece { color: Color::Red, piece_type: PieceType::Pawn });
        }
        ...
        for y in 0..10 {
            ...
            for x in 0..9 {
                let c = match self.cells[y][x] {
                    Some(Piece { color: Color::Red, piece_type: PieceType::King }) => "帥",
                    Some(Piece { color: Color::Red, piece_type: PieceType::Advisor }) => "仕",
                    Some(Piece { color: Color::Red, piece_type: PieceType::Elephant }) => "相",
                    Some(Piece { color: Color::Red, piece_type: PieceType::Horse }) => "馬",
                    Some(Piece { color: Color::Red, piece_type: PieceType::Chariot }) => "車",
                    Some(Piece { color: Color::Red, piece_type: PieceType::Cannon }) => "炮",
                    Some(Piece { color: Color::Red, piece_type: PieceType::Pawn }) => "兵",
                    Some(Piece { color: Color::Black, piece_type: PieceType::King }) => "將",
                    Some(Piece { color: Color::Black, piece_type: PieceType::Advisor }) => "士",
                    Some(Piece { color: Color::Black, piece_type: PieceType::Elephant }) => "象",
                    Some(Piece { color: Color::Black, piece_type: PieceType::Horse }) => "馬",
                    Some(Piece { color: Color::Black, piece_type: PieceType::Chariot }) => "車",
                    Some(Piece { color: Color::Black, piece_type: PieceType::Cannon }) => "砲",
                    Some(Piece { color: Color::Black, piece_type: PieceType::Pawn }) => "卒",
                    None => "·",
                };
                write!(f, "{} ", c)?;
            }
...
mod eval {
    ...
    fn position_value(pt: PieceType, c: Color, x: u8, y: u8) -> i32 {
        ...
        let val = match pt {
            PieceType::Pawn => if c == Color::Red { PST_PAWN_RED[y_usize][x_usize] } else { PST_PAWN_RED[y_mirrored][x_usize] },
            PieceType::Horse => if c == Color::Red { PST_HORSE_RED[y_usize][x_usize] } else { PST_HORSE_RED[y_mirrored][x_usize] },
            PieceType::Chariot => if c == Color::Red { PST_CHARIOT_RED[y_usize][x_usize] } else { PST_CHARIOT_RED[y_mirrored][x_usize] },
            _ => 0,
        };
...
mod search {
    ...
    pub fn alpha_beta(
        ...
        if moves.is_empty() {
            return -100000 + (4 - depth as i32); // 输了，但晚输比早输好
        }
        ...
        for action in moves {
            ...
            let eval = if illegal {
                ...
                -alpha_beta(board, depth - 1, -beta, -alpha, side.opponent(), &mut None)
            };
    ...
    pub fn find_best_move(board: &mut Board, depth: u8, side: Color) -> Option<Action> {
        ...
        let _ = alpha_beta(board, depth, -100000, 100000, side, &mut best_action);
        best_action
    }
}
...
fn main() {
    ...
    loop {
        // AI走 (红方)
        println!("AI思考中...");
        if let Some(action) = search::find_best_move(&mut board, 4, Color::Red) {
            println!("AI走: ({}, {}) -> ({}, {})", action.src.x, action.src.y, action.tar.x, action.tar.y);
            board.make_move(action);
            println!("{}", board);
        } else {
            println!("AI无子可走，你赢了！");
        ...
        let x1 = match parts[0].parse::<u8>() { Ok(v) => v, Err(_) => { println!("无效坐标"); continue; } };
        let y1 = match parts[1].parse::<u8>() { Ok(v) => v, Err(_) => { println!("无效坐标"); continue; } };
        let x2 = match parts[2].parse::<u8>() { Ok(v) => v, Err(_) => { println!("无效坐标"); continue; } };
        let y2 = match parts[3].parse::<u8>() { Ok(v) => v, Err(_) => { println!("无效坐标"); continue; } };
        ...
        let piece = match board.get(src) {
            Some(p) if p.color == Color::Black => p,
            _ => { println!("该位置没有你的棋子"); continue; }
        };
